...
ALPHA_MIN = 50
ALPHA_MAX = 340
ball_image_path = [os.path.join(current_dir, 'white_ball.png'),
            os.path.join(current_dir, 'red_ball.png'),
            os.path.join(current_dir, 'green_ball.png'),
            os.path.join(current_dir, 'blue_ball.png'),
            os.path.join(current_dir, 'yellow_ball.png')]
...
def next_image():
    ...
    scaled_image = pygame.image.load(image_path)
...
class Particle:
    cached_images = {}
    def __init__(self, pos, color, size, alpha, alpha_dir):
    ...
    def update_image(self):       
        ...
        ball_image = pygame.Surface((self.size, self.size), pygame.SRCALPHA)
        scaled_image = pygame.transform.scale(pygame.image.load(ball_image_path[self.color]),(self.size, self.size)) 
        ball_image.blit(scaled_image, (0,0))       
...
def main():
    ...
    pm = ParticleManager()    
    for _ in range(40): # until it slow down under 60Hz    
        # top
        pm.add(Particle((random.uniform(0, screen_size.x), random.uniform(0, 60)), #position
        ...
        #bottom
        pm.add(Particle((random.uniform(0, screen_size.x), random.uniform(screen_size.y-60, screen_size.y)), #position
                            #(0, random.uniform(20, 100)), #velocity
                            random.randint(0,4),
                            random.randint(20, 40), #size
                            random.randint(ALPHA_MIN, ALPHA_MAX), #alpha
    ...
    for _ in range(20): # until it slow down under 60Hz  
        #left
        pm.add(Particle((random.uniform(0, 120), random.uniform(60, screen_size.y -60)), #position
                            #(0, random.uniform(20, 100)), #velocity
                            random.randint(0,4),
                            random.randint(20, 40), #size
                            random.randint(ALPHA_MIN, ALPHA_MAX), #alpha
        ...
        #right
        pm.add(Particle((random.uniform(screen_size.x-120, screen_size.x), random.uniform(60, screen_size.y -60)), #position
                            #(0, random.uniform(20, 100)), #velocity
                            random.randint(0,4),
                            random.randint(20, 40), #size
                            random.randint(ALPHA_MIN, ALPHA_MAX), #alpha
